...
Any custom backend for log store should implement the `LogStore` interface. The interface is defined in [logstore/store.go](https://github.com/grevinden/bifrost/blob/main/framework/logstore/store.go).
...
GORM provides implementations for the functions listed in the `LogStore` interface. This significantly simplifies the implementation of the log store (see [postgres.go](https://github.com/grevinden/bifrost/blob/main/framework/logstore/postgres.go) as an example).
