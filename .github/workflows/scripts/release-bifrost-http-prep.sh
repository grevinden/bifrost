...
done < <(grep "github.com/grevinden/bifrost/plugins/" go.mod)
cd ..
...
for plugin_name in "${!PLUGIN_VERSIONS[@]}"; do
  ...
  if grep -q "github.com/grevinden/bifrost/plugins/$plugin_name" go.mod; then
    echo "  📦 Using $plugin_name plugin $plugin_version"
    # Textual require bump — skips loading the currently-declared version's go.mod
    go mod edit -require="github.com/grevinden/bifrost/plugins/$plugin_name@$plugin_version"
  fi
done
...
go mod edit -require="github.com/grevinden/bifrost/core@$CORE_VERSION"

echo "  📦 Updating framework to $FRAMEWORK_VERSION"
go mod edit -require="github.com/grevinden/bifrost/framework@$FRAMEWORK_VERSION"
