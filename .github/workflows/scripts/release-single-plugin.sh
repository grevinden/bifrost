...
if [ -f "go.mod" ]; then
  go_get_with_backoff "github.com/grevinden/bifrost/core@${CORE_VERSION}"
  go_get_with_backoff "github.com/grevinden/bifrost/framework@${FRAMEWORK_VERSION}"
  go mod tidy
...
go get github.com/grevinden/bifrost/plugins/$PLUGIN_NAME@v$PLUGIN_VERSION
\`\`\`
